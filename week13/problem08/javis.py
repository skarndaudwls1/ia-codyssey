'''
javis.py
시스템 마이크로 음성을 녹음하고, 녹음된 파일을 텍스트로 옮겨(STT) CSV 로
저장하는 도구.
...
- STT(Speech to Text) 부분에 한해 외부 라이브러리(SpeechRecognition)
  사용이 허용된다. 없으면 STT 기능만 비활성화하고 나머지는 동작한다.
- 녹음 파일은 본 스크립트와 같은 위치의 records 폴더에 저장한다.
  파일 이름은 녹음 시각을 참조해 '년월일-시간분초.wav' 형식으로 만든다.
  예) 20260516-143012.wav
- STT 결과는 음성 파일과 같은 이름에 확장자만 .CSV 로 바꿔 같은 폴더에
  저장한다. 예) 20260516-143012.wav -> 20260516-143012.CSV
  CSV 의 각 줄은 '음성 파일 내에서의 시간, 인식된 텍스트' 두 칸으로 구성된다.
- 보너스: 특정 키워드로 저장된 CSV 들을 검색해 일치하는 줄을 보여준다.
...
try:
    # speech_recognition 은 import 시 표준 라이브러리 aifc(파이썬 3.13에서
    # 제거 예정)를 불러오며 DeprecationWarning 을 낸다. 우리 코드와 무관한
    # 라이브러리 내부 경고이므로, '경고 없이 실행' 규칙을 지키기 위해
    # import 동안만 좁게 가린다. (기본 실행에서는 어차피 표시되지 않는다.)
...
# STT 관련 상수.
STT_LANGUAGE = 'ko-KR'        # Google STT 에 넘길 인식 언어.
STT_SEGMENT_SECONDS = 10      # 음성을 이 길이(초)로 잘라 구간별로 인식한다.
CSV_EXTENSION = '.CSV'
CSV_HEADER = ('시간', '인식된 텍스트')
CSV_ENCODING = 'utf-8-sig'    # 엑셀에서 한글이 깨지지 않도록 BOM 포함 UTF-8.
...
# ---------------------------------------------------------------------------
# STT 백엔드 (Speech to Text)
# ---------------------------------------------------------------------------
class GoogleSpeechRecognizer:
    '''SpeechRecognition 라이브러리의 Google Web Speech API 기반 STT 백엔드.

    SpeechRecognition 의 AudioFile 로 음성 파일을 열어 STT_SEGMENT_SECONDS
    초 단위로 끊어 읽고, 각 구간을 Google STT 로 인식한다. AudioFile 이
    채널 수/샘플 폭/포맷(WAV·AIFF·FLAC)을 알아서 정규화하므로 모노가
    아니거나 샘플 폭이 달라도 처리된다. 구간 시작 시각과 인식된 텍스트의
    짝을 차례로 돌려준다.
    '''
    ...
    def _recognize(self, audio_data):
        '''한 구간(AudioData)을 텍스트로 바꾼다.

        말소리가 없어 인식할 내용이 없으면 빈 문자열을 돌려준다.
        네트워크/API 오류(sr.RequestError)는 그대로 올려보내 호출 측이
        파일 단위로 처리하게 한다.
        '''
        try:
            return self._recognizer.recognize_google(
                audio_data, language=STT_LANGUAGE)
        except _sr.UnknownValueError:
            return ''

    def transcribe_segments(self, path):
        '''음성 파일을 구간별로 인식해 (시작_시간_초, 텍스트) 목록을 돌려준다.'''
        if STT_SEGMENT_SECONDS <= 0:
            return []
        rows = []
        with _sr.AudioFile(path) as source:
            start_seconds = 0
            while True:
                segment = self._recognizer.record(
                    source, duration=STT_SEGMENT_SECONDS)
                # 파일 끝에 닿으면 빈 구간이 돌아온다.
                if len(segment.frame_data) == 0:
                    break
                rows.append((start_seconds, self._recognize(segment)))
                start_seconds += STT_SEGMENT_SECONDS
        return rows


def _select_stt_engine():
    '''사용 가능한 STT 백엔드를 골라 돌려준다. 없으면 None.'''
    if _sr is not None:
...
def _list_record_files():
    '''records 폴더의 녹음(WAV) 파일 이름을 정렬해 돌려준다.'''
...
def _list_csv_files():
    '''records 폴더의 STT 결과(CSV) 파일 이름을 정렬해 돌려준다.'''
    if not os.path.isdir(RECORDS_DIR):
        return []
    names = []
    for name in os.listdir(RECORDS_DIR):
        if name.lower().endswith(CSV_EXTENSION.lower()):
            names.append(name)
    return sorted(names)


def _csv_path_for(wav_name):
    '''WAV 파일 이름에 대응하는 CSV 경로를 만든다.

    확장자만 .CSV 로 바꾸며, 원래 이름은 그대로 유지한다.
    '''
    stem = wav_name
    if stem.lower().endswith(FILE_EXTENSION):
        stem = stem[:-len(FILE_EXTENSION)]
    return os.path.join(RECORDS_DIR, stem + CSV_EXTENSION)


def _format_timestamp(total_seconds):
    '''초 단위 정수를 'HH:MM:SS' 문자열로 바꾼다.'''
...
def _write_transcript_csv(csv_path, rows):
    '''(시작_시간_초, 텍스트) 목록을 CSV 로 저장한다.

    첫 줄은 머리글, 이후 줄은 'HH:MM:SS, 인식된 텍스트' 형식이다.
    '''
    with open(csv_path, 'w', encoding=CSV_ENCODING, newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(CSV_HEADER)
        for start_seconds, text in rows:
            writer.writerow((_format_timestamp(start_seconds), text))


def _read_transcript_csv(csv_path):
    '''CSV 를 읽어 (시간문자열, 텍스트) 목록을 돌려준다. 머리글은 건너뛴다.'''
    rows = []
    with open(csv_path, 'r', encoding=CSV_ENCODING, newline='') as csv_file:
...
# ---------------------------------------------------------------------------
# 기능 4. 음성 → 텍스트 변환(STT) 후 CSV 저장
# ---------------------------------------------------------------------------
def _transcribe_file(engine, wav_name):
    '''WAV 한 개를 STT 로 변환하고 같은 이름의 CSV 로 저장한다.

    저장한 CSV 경로와 인식된 줄 수를 (csv_path, 줄수) 로 돌려준다.
    '''
    wav_path = os.path.join(RECORDS_DIR, wav_name)
    rows = engine.transcribe_segments(wav_path)
    csv_path = _csv_path_for(wav_name)
    _write_transcript_csv(csv_path, rows)
    return csv_path, len(rows)


def _run_stt(engine):
    if engine is None:
        _print_warn('STT 라이브러리(SpeechRecognition)가 없어 변환할 수 없습니다.')
        return

    _print_section('음성 → 텍스트 변환(STT)')

    wav_names = _list_record_files()
    if not wav_names:
        _print_warn('변환할 녹음 파일이 없습니다. 먼저 녹음을 해주세요.')
        return

    print('  녹음 파일 목록')
    for index, name in enumerate(wav_names, start=1):
        csv_exists = os.path.isfile(_csv_path_for(name))
        mark = ' (CSV 있음)' if csv_exists else ''
        print('  [{0:>2}] {1}{2}'.format(index, name, mark))

    print()
    print('  변환할 파일 번호를 입력하세요. (전체: a, 취소: 빈 입력)')
    answer = _ask('  > ')
    ...
    if answer.lower() == 'a':
        targets = wav_names
    elif answer.isdigit() and 1 <= int(answer) <= len(wav_names):
        targets = [wav_names[int(answer) - 1]]
    else:
        _print_warn('올바른 번호 또는 a 를 입력해 주세요.')
        return

    for name in targets:
        print()
        print('  변환 중 : {0}'.format(name))
        try:
            csv_path, line_count = _transcribe_file(engine, name)
        except (wave.Error, ValueError) as error:
            # AudioFile 은 WAV/AIFF/FLAC 가 아니거나 손상된 파일에
            # ValueError 를 낸다 (mp3/m4a 등 압축 포맷 포함).
            _print_error('음성 파일을 읽을 수 없습니다 ({0}): {1}'.format(
                name, error))
            continue
        except OSError as error:
            _print_error('파일 처리 중 오류가 발생했습니다 ({0}): {1}'.format(
                name, error))
            continue
        # SpeechRecognition 의 네트워크/API 오류 등은 한 파일만 건너뛴다.
        except Exception as error:
            _print_error('STT 변환 중 오류가 발생했습니다 ({0}): {1}'.format(
                name, error))
            continue

        print('  저장 완료 : {0}  (구간 {1}개)'.format(
            os.path.basename(csv_path), line_count))


# ---------------------------------------------------------------------------
# 기능 5. (보너스) 키워드로 CSV 검색
# ---------------------------------------------------------------------------
def _search_keyword_in_csvs(keyword):
    '''저장된 모든 CSV 에서 키워드가 포함된 줄을 찾는다.

    (csv_파일이름, 시간문자열, 텍스트) 목록을 돌려준다. 대소문자는 무시한다.
    '''
    matches = []
    lowered = keyword.lower()
    for csv_name in _list_csv_files():
        csv_path = os.path.join(RECORDS_DIR, csv_name)
        try:
            rows = _read_transcript_csv(csv_path)
        except OSError:
            continue
        for time_text, text in rows:
            if lowered in text.lower():
                matches.append((csv_name, time_text, text))
    return matches


def _search_transcripts():
    _print_section('키워드로 STT 결과(CSV) 검색')

    if not _list_csv_files():
        _print_warn('검색할 CSV 가 없습니다. 먼저 STT 변환을 해주세요.')
        return

    keyword = _ask('  찾을 키워드 (취소: 빈 입력) > ')
    if keyword is None or keyword == '':
        print('  검색을 취소했습니다.')
        return

    matches = _search_keyword_in_csvs(keyword)
    if not matches:
        _print_warn("'{0}' 을(를) 포함한 내용을 찾지 못했습니다.".format(keyword))
        return

    print()
    print('  찾은 줄 : {0}개'.format(len(matches)))
    for csv_name, time_text, text in matches:
        print('  - {0} | {1} | {2}'.format(csv_name, time_text, text))
...
def _print_menu():
    ...
    print('  4. 음성 → 텍스트 변환(STT) 후 CSV 저장')
    print('  5. 키워드로 CSV 검색 (보너스)')
    print('  0. 종료')
...
def _main():
    ...
    engine = _select_stt_engine()
    if engine is None:
        _print_warn('STT 라이브러리(SpeechRecognition)를 찾지 못했습니다.')
        _print_warn('STT 기능은 비활성화되며, 나머지 기능은 사용할 수 있습니다.')
        print('      설치 예) pip install SpeechRecognition')
    else:
        print('  STT 백엔드  : {0}'.format(engine.name))
    ...
    while True:
        ...
        if choice == '1':
            ...
            _print_warn('1, 2, 3, 4, 5, 0 중에서 선택해 주세요.')
