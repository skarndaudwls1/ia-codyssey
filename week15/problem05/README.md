...
- `.csv` 파일이 여러 개일 수 있으므로, **한 개든 여러 개든 항상 목록을
  보여주고** 사용할 파일을 번호로 직접 고른다(`0` 은 취소).
- 한 개도 없으면 안내만 하고 메뉴로 돌아간다.

```
현재 폴더에서 찾은 CSV 파일입니다. 어떤 파일을 읽을까요?
  1. mars_weathers_data.CSV
  0. 취소
CSV 선택>
```
...
먼저 **헤더**를 확인한다. 컬럼 이름·순서가 기대값
(`weather_id,mars_date,temp,stom`)과 다르면, 위치 기반 파싱이 엉뚱한
값을 넣을 수 있으므로 그 파일은 반영하지 않는다.

이어서 각 **행**이 테이블 속성과 맞는지 확인한다.

- `mars_date` 는 `YYYY-MM-DD` 형식의 **실제 존재하는 날짜**여야 한다
  (DATETIME). 월별 일수와 윤년까지 따져 `2050-02-31` 같은 값은 거른다.
- `temp`, `storm` 은 정수로 바꿀 수 있어야 한다(INT). `temp` 의 소수
  값은 `round()` 로 반올림한다.

헤더가 다르거나 한 행이라도 맞지 않으면 **일부만 넣지 않고 그 파일
전체를 반영하지 않는다.** 어떤 줄이 왜 문제인지 안내한 뒤 메뉴로
돌아간다(부분 적재로 데이터가 어긋나는 것을 막기 위함).
...
- 재실행 시 데이터 중복을 막기 위해 입력 전에 테이블을 `TRUNCATE` 로
  비우고 새로 넣는다(덮어쓰기). 같은 CSV 를 다시 넣으면 행 수가 똑같아
  보이므로, 입력 후 "기존 N개를 비우고 M개를 새로 입력" 처럼 전/후 행
  수를 함께 보여준다.
