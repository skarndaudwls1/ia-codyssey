...
4. 테이블 내용 조회        # 적재된 전체 행을 표 형태로 보여준다
0. 종료
```
...
## 데이터 검증 (DB 구조 동적 비교, 전체 반영 방식)

컬럼 목록을 코드에 고정하지 않는다. 3번(입력)을 누르면 **그 시점에 실제
테이블 구조를 `SHOW COLUMNS` 로 읽어** CSV 가 거기에 맞는지 판단한다.
그래서 테이블 구조가 바뀌어도 코드를 고칠 필요가 없다.

판단 순서:

1. **구조(컬럼 수) 비교** — CSV 컬럼 수가 테이블 전체 컬럼 수(자동증가
   포함)와 같으면 자동증가 컬럼 자리는 건너뛰고, 자동증가 제외 컬럼
   수와 같으면 그대로 매핑한다. 둘 다 아니면 "구조가 맞지 않아 반영할
   수 없습니다" 하고 돌아간다.
2. **값 ↔ 타입 비교** — 각 값을 해당 컬럼의 **타입**에 맞춰 변환해 본다.
   - 정수 계열(`INT` 등): 숫자여야 한다. 소수 값은 `round()` 로 반올림.
   - 실수 계열(`FLOAT`/`DOUBLE`/`DECIMAL`): 실수여야 한다.
   - 날짜 계열(`DATETIME`/`DATE`): `YYYY-MM-DD` 형식의 **실제 존재하는
     날짜**여야 한다(월별 일수·윤년까지 따져 `2050-02-31` 은 거른다).
   - 문자열 계열(`VARCHAR`/`TEXT`): 그대로 사용.

이름이 아니라 **위치 + 타입**으로 맞추므로, 헤더의 `stom`(storm 오타)
처럼 이름이 달라도 적재된다. 한 행이라도 타입에 맞지 않으면 **일부만
넣지 않고 그 파일 전체를 반영하지 않고**, 어떤 줄이 왜 문제인지 안내한
뒤 메뉴로 돌아간다(부분 적재로 데이터가 어긋나는 것을 막기 위함).

> 트레이드오프: 이름을 보지 않으므로 우연히 순서가 같고 의미가 다른
> CSV 도 받아들일 수 있다. 또 검증에 DB 가 필요하므로, 테이블이 없으면
> (2번 미실행) 먼저 만들어야 한다.

## 동작 메모

- 적재 컬럼/타입은 실행 시점의 실제 테이블 구조를 따른다. `weather_id`
  같은 자동 증가(AUTO_INCREMENT) 컬럼은 DB 가 채우므로 INSERT 에서
  제외한다.
- CSV 헤더의 마지막 컬럼명 `stom` 은 `storm` 의 오타지만, 위치 기반
  매핑이라 이름이 달라도 그대로 `storm` 컬럼에 들어간다.
