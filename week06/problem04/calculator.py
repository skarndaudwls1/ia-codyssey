'''아이폰 17 Pro 스타일 계산기 — Calculator 클래스로 로직 분리.

역할별 분리:
  CalculatorDesign  : 아이폰 외형 그리기 (프레임·노치·상태바 등)
  Calculator        : 계산기 상태와 연산 (UI와 무관, 단위 테스트 가능)
  CalculatorScreen  : 버튼·디스플레이 같은 UI 위젯
  IPhoneFrame       : 바깥 창 + 이벤트(드래그·키보드·닫기)

명세에 따른 Calculator 외부 메소드:
  사칙연산 입력  : add(), subtract(), multiply(), divide()
  부가기능       : reset(), negative_positive(), percent(), equal()
  입력           : input_digit(d), input_dot(), backspace()
'''
...
# ── 계산기 로직 상수 ──
MAX_INPUT_DIGITS = 15            # 한 피연산자에 입력 가능한 최대 자릿수
ROUND_DECIMALS = 6               # 보너스: 소수점 6자리 이하 반올림
SCI_UPPER = 1e16                 # 이 이상 절댓값은 과학적 표기로 전환
SCI_LOWER = 1e-6                 # 이 미만 절댓값(0 제외)도 과학적 표기로 전환
...
class Calculator:
    '''계산기 코어 — 상태와 연산을 담당하고 UI는 모른다.

    명세 요구사항:
      * add/subtract/multiply/divide  : 사칙연산 입력
      * reset                         : 초기화
      * negative_positive             : 부호 전환
      * percent                       : 백분율
      * equal                         : 결과 출력
      * 숫자키 누를 때마다 누적 입력
      * 소수점 중복 입력 방지
      * 0 나누기·오버플로·NaN 등 수학 예외는 '오류'로 표시
      * 결과는 소수점 6자리 이하 반올림 (보너스)
      * 너무 크거나 작은 값은 과학적 표기로 자동 전환

    UI(CalculatorScreen)는 위 메소드를 호출해 상태를 바꾸고,
    ...
    def history_text(self):
        ...
        preview = self._compute(self._first, op, second)
        if preview is None:
            return f'{base} {op} {self._display}'
    ...
    def percent(self):
        ...
        if self.is_error:
            return
        try:
            value = float(self._strip(self._display)) / 100
        except (ValueError, OverflowError):
            self._display = '오류'
            return
        self._display = self._fmt(value)

    def equal(self):
        '''= — 대기 중인 연산을 수행하고 결과 표시.'''
    ...
    def _resolve(self, final):
        ...
        try:
            ...
            if final:
                self._prefix = ''
            return
        result = self._compute(self._first, self._op, second)
        if result is None:
            self._display = '오류'
        else:
    ...
    @staticmethod
    def _compute(a, op, b):
        '''두 숫자와 연산자로 결과를 얻는다. 수학 예외는 None 으로.

        처리하는 예외:
          * 0으로 나누기   → None
          * 오버플로       → None (1e308 * 1e10 등)
          * NaN/무한대     → None
          * 미지의 연산자  → None
        '''
        try:
    ...
    def _fmt(self, value):
        ...
        규칙:
          1) 너무 크거나(>=1e16) 너무 작은(0 제외, <1e-6) 값은
             과학적 표기 (예: 9.0000E+15)
          2) 그 외는 소수점 ROUND_DECIMALS 자리에서 반올림 (보너스)
          3) 정수 결과는 소수점 없이 천 단위 콤마로 표시
        '''
        # 1) 과학적 표기 영역
        ...
        if rounded == int(rounded):
            return self._format_number(str(int(rounded)))
        # 소수부 ROUND_DECIMALS 자리까지만 남기고 뒤의 0은 제거
        raw = f'{rounded:.{ROUND_DECIMALS}f}'.rstrip('0').rstrip('.')
        if raw in ('', '-'):
            raw = '0'
...
class CalculatorScreen(QWidget):
    '''아이폰 화면 안쪽의 위젯 — 버튼 배치와 이벤트 처리.

    계산은 self._calc(Calculator)에 위임한다.
    ...
    # ── 사용자 입력 → Calculator 호출 ──

    # 연산자 → Calculator 메소드 매핑 (명세 메소드명 그대로 사용)
    _OP_DISPATCH = {
        '+': 'add',
        '-': 'subtract',
        '×': 'multiply',
    ...
    def _on_button(self, text):
        ...
        if self._calc.is_error and text != 'AC':
            return

        if text.isdigit():
            ok = self._calc.input_digit(text)
            if not ok:
                self._show_max_digit_notice()
            self._highlight_operator(None)
        elif text == '.':
            self._calc.input_dot()
            self._highlight_operator(None)
        elif text in OPERATORS:
            getattr(self._calc, self._OP_DISPATCH[text])()
    ...
    def _sync_ui(self):
        # 디스플레이 — 텍스트 길이에 맞춰 글자 크기 자동 조정 (보너스)
        ...
        # 수식 표시줄
        self._set_history_text(self._calc.history_text())

        # 백스페이스 버튼은 의미 있는 상태에서만 활성화
        backspace_btn = self._buttons.get('⌫')
        if backspace_btn is not None:
            backspace_btn.setEnabled(self._calc.can_backspace())
...
class IPhoneFrame(QWidget):
    ...
    외형은 CalculatorDesign, 계산은 Calculator에 각각 위임한다.'''
